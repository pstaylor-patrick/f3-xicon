    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {selectedTags.length > 0 && (
            <span className="ml-1 rounded-full bg-primary px-2 py-0.5 text-xs text-white">{selectedTags.length}</span>
          )}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-80">
        ...
        <div className="mt-4 flex items-center space-x-2">
          ...
          <Label htmlFor="operator-mode">{isAnd ? "Match ALL tags (AND)" : "Match ANY tag (OR)"}</Label>
        </div>
